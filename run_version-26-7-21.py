...
def load_pretrained_checkpoint(
    ...
    if checkpoint["classifier_type"] != args.classifier_type:
        raise ValueError(
            f"Checkpoint classifier mismatch: expected={args.classifier_type}, "
            f"actual={checkpoint['classifier_type']}"
        )
...
def build_models(args, device):
    ...
    if args.classifier_type == "stochastic_classifier":
        ...
        raise ValueError(f"Unsupported classifier: {args.classifier_type}")
...
def image_test_classification(loader, base_model, classifiers, args):
    start_test = True
    base_model.eval()
    classifiers.eval()
...
def train(args):
    ...
    # for I/Q signal 
    SD_loader, TD_loader, test_loader = make_loaders(args, augment_config)

    # for matlab GASF
    # SD_lines, _ = read_lines_and_validate(args.SD_list.resolve(), args)
    # TD_lines, _ = read_lines_and_validate(args.TD_list.resolve(), args)
    # SD_loader = make_loader_22222(SD_lines, image_train_transform(), args, shuffle=True, drop_last=True)
    # TD_loader = make_loader_22222(TD_lines, image_train_transform(), args, shuffle=True, drop_last=True)
    # test_loader = make_loader_22222(TD_lines, image_test_transform(), args, shuffle=False, drop_last=False)
    ...
    checkpoint_path = (
        Path(args.save_dir)
        / args.net
        / f"TD_SNR_{int(args.t)}_class_{args.TD_class_num}_{args.classifier_type}_{timestamp}.pt"
    )
...
def build_parser():
    ...
    parser.add_argument("--pretrain_model_path", type=Path,
                        ...
                        default=Path(
                            ...
                            r"\model\RepVGG_B1g2"
                            r"\SD_SNR_20_class_50_pretrain_stochastic_classifier_20260720_152722.pt"
                            ),
