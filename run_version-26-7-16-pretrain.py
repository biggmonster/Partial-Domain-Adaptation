...
def make_loaders(args, augment_config):
    ...
    validation_dataset = data_list.MatSourceGASFDataset(
        validation_records,
        mat_path=args.raw_mat_path,
        transform=transform,
        target_size=args.gasf_size,
        seed=args.seed,
        samples_per_class=args.samples_per_class,
        augment_config=augment_config,
        contrastive=False,
...
def build_models(args, device):
    ...
    projection_head = en_network.ContrastiveProjectionHead(
        input_dim=base_network.embedding_dim,
        hidden_dim=base_network.embedding_dim,
...
def evaluate_source(loader, base_network, classifier, device, amp_enabled):
    base_network.eval()
    classifier.eval()
    correct = 0
    total = 0
    with torch.no_grad():
        for images, labels, _ in loader:
            ...
            with torch.cuda.amp.autocast(enabled=amp_enabled):
                ...
                logits = classifier(features, mode="test")
...
def save_checkpoint(
    ...
    checkpoint = {
        ...
        "projection_head": copy.deepcopy(projection_head.state_dict()),
...
def train(args):
    ...
    base_network, classifier, projection_head = build_models(args, device)

    parameter_list = (
        base_network.get_parameters()
        + classifier.get_parameters()
        + projection_head.get_parameters()
    )
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        for original, view_one, labels, _ in progress:
            ...
            with torch.cuda.amp.autocast(enabled=amp_enabled):
                original_fea, _, _ = base_network(original)
                ce_loss = classifier_loss(
                    classifier, original_fea, labels, args.classifiers_num
                )

                augmented_fea, embeddings, _ = base_network(augmented)
                projections = projection_head(embeddings)
                projection_one, projection_two = projections.chunk(2, dim=0)
                contrast_loss = my_loss.NTXentLoss(
                    projection_one,
                    projection_two,
                    temperature=args.temperature,
                )
                weighted_contrast_loss = args.contrast_weight * contrast_loss
            scaler.scale(weighted_contrast_loss).backward()
            ...
            progress.set_postfix(
                ...
                ntxent=f"{contrast_total / batches:.4f}",
            )
        ...
        print(
            ...
            f"NT-Xent={contrast_total / max(1, batches):.6f}, "
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Source-only CE + signal-level NT-Xent pretraining"
    )
