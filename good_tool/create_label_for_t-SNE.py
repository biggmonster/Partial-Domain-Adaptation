...
def generate_source_label_files(root_dir, styles, output_dir, source_samples_num):
    """
    ...
    for style in styles:
        ...
        output_txt = os.path.join(output_dir, f"{style}_200_list.txt")   # 生成标签文件的绝对路径
        samples = []
        
        # 遍历指定类别
        random_class_ids =[ 48, 40, 31, 29, 43, 15, 22, 46, 1, 3]
...
def generate_target_label_files(root_dir, styles, output_dir, target_samples_num, target_class_num):
    """
    为ADS-B数据集的每个风格生成标签文件
    ...
    for style in styles:
        ...
        output_txt = os.path.join(output_dir, f"{style}_{target_class_num}_200_list.txt")   # 生成标签文件的绝对路径
        samples = []
        i = 0
...
#----------------------------LongSig_50------------------------------------------------------------------- 
    ...
    source_output_dir = r"D:\LJ\workstation\Vscode\New_ISRA\data\LongSig_50\t-SNE\Source\class_10"         # 标签文件存放的路径

    target_root_dir   = r"D:\LJ\workstation\Matlab\ads-b_data_gen\data_LongSig_50\target\gasf_old"           # 数据集图片文件夹路径
    target_output_dir = r"D:\LJ\workstation\Vscode\New_ISRA\data\LongSig_50\t-SNE\Target\class_10"         # 标签文件存放的路径
...
#-----------源域50类，目标域30类-----------------------------------------------------------------     
    num_classes = 50                       # 
    source_samples_num = 200
    target_samples_num = 200               # 
    target_class_num = 30
#----------------------------------------------------------------------------
   
    # 生成标签文件
    generate_source_label_files(source_root_dir, styles, source_output_dir, source_samples_num)

    generate_target_label_files(target_root_dir, styles,target_output_dir, target_samples_num, target_class_num)
