...
def load_class_ids():
    raw_class_ids = os.getenv("CLASS_IDS")
...
def parse_args():
    ...
    parser.add_argument(
        "--plots-dir",
        type=Path,
        default=project_root/ "t-SNE_results"/ "pretrained_resnet50"/ "class_5",
    )
...
def legend_handles():
    ...
    target_handles = [
        Line2D(
            ...
            markersize=6,
            alpha=1,
            label=f"Target Class {class_id}",
        )
        for class_id in CLASS_IDS
    ]
    return source_handles + target_handles
...
def draw_figures(
    plots_dir, source_points, source_labels, target_points, target_labels, figure_title
):
