...
def metrics_payload(pair, class_ids, class_num, label_counts, confusion, run_timestamp):
    names = display_names(class_ids)
    outside_by_true = {
        names[index]: {
            "class_id": class_id,
...
def process_pair(pair, class_ids, args, device):
    ...
    y_true, y_pred = collect_predictions(model, loader, device, class_ids, class_num)
    ...
    metrics = metrics_payload(
        pair, class_ids, class_num, label_counts, confusion, args.run_timestamp
    )
    write_json(
