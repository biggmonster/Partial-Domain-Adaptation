...
def generate_target_label_files(root_dir, styles, num_classes, output_dir, target_samples_num, target_class_num):
    ...
    for style in styles:
        ...
        for class_id in thirty_class_ids:
            if class_id  < 9: 
                class_dir = os.path.join(style_dir, f"class_0{class_id+1}")
            else:
                class_dir = os.path.join(style_dir, f"class_{class_id+1}")
...
#----------------------------LongSig_50------------------------------------------------------------------- 
    ...
    target_output_dir = r"D:\LJ\workstation\Vscode\New_ISRA\data\LongSig_50\Target\Multi_class"         # 标签文件存放的路径
...
# ---------------------------preamble----------------------------------------------------------------------
    ...
    #           ]    
    styles = [
                'SNR_-5',  'SNR_5', 'SNR_10',  'SNR_15', 'SNR_20' 
