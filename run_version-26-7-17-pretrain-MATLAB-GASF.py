...
def stratified_split_lines(lines, labels, validation_ratio, seed):
    """Split every source class reproducibly while preserving list order."""

    if len(lines) != len(labels):
        raise ValueError(
            f"lines/labels length mismatch: {len(lines)} lines, "
            f"{len(labels)} labels"
        )
    ...
    for label in sorted(grouped_indices):
        class_indices = grouped_indices[label][:]
        valid_count = int(len(class_indices) * validation_ratio)
        if valid_count < 1 or valid_count >= len(class_indices):
            raise ValueError(
                f"Class {label} has {len(class_indices)} samples, which cannot "
                f"be split with validation_ratio={validation_ratio}"
            )

        rng.shuffle(class_indices)
        valid_indices.extend(class_indices[:valid_count])
        train_indices.extend(class_indices[valid_count:])

    # Randomize membership only. Keeping the original source-list order inside
    # each subset makes the selected samples straightforward to inspect.
    train_indices.sort()
    valid_indices.sort()
    train_lines = [lines[index] for index in train_indices]
    valid_lines = [lines[index] for index in valid_indices]
...
def train(args):
    ...
    # train_loader, validation_loader = make_loaders(args, augment_config)

    lines, labels = read_lines_and_validate(args.source_list.resolve())
    train_lines, valid_lines = stratified_split_lines(
        lines,
        labels,
    ...
    train_loader = make_loader(train_lines, transform, args, shuffle=True)
    valid_loader = make_loader(valid_lines, transform, args, shuffle=False)
    print(
        f"Stratified source split (seed={args.seed}): "
        f"train={len(train_lines)}, validation={len(valid_lines)}, "
        f"ratio={1.0 - args.source_val_ratio:.1%}:"
        f"{args.source_val_ratio:.1%}"
    )
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        for original, labels, _ in progress:
            ...
            with torch.cuda.amp.autocast(enabled=amp_enabled):       # 使用混合精度，自动选择计算精度
                ...
                original_features, _, _ = base_network(original)
...
def build_parser():
    ...
    parser.add_argument("--source_val_ratio", type=float, default=0.1)
    ...
    parser.add_argument(
        ...
        help='MATLAB-GASF source list file, each line: "mat_index label"',
    )
